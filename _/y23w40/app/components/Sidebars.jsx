...
import { SidebarsList01, SvgPaths, LinkSvg, DropBox } from "./B5Elements"
...
export default ({ bem = "B5-Sidebars", align = "sidebars --dark --collapsible --foldableMenuItems", $datas }) => {
    ...
    return <aside className={bem + ' ' + align}>
        <a href={$datas.logoAnchor[0]} >
            <SvgPaths viewBox="0 0 118 94" pathsArray={$datas.logoAnchor[2]}/>
            <span>{$datas.logoAnchor[1]}</span>
        </a>
        <hr />
        <ul>
            {$datas.list.map((item,i) => <li>
                <a href={item[0]} className={i==0?"active":""} aria-current={i==0?"page":false}>
                    <SvgPaths viewBox="0 0 16 16" pathsArray={item[2]}/>
                    <label htmlFor={"okok"+i}>{item[1]}</label>
        ...
        <div>
            ...
            <ul>
                {$datas.dropup.list.map((item,i) => <li><a href={item[0]}>{item[1]}</a></li>)}
            </ul>
        </div>
    </aside>
}
