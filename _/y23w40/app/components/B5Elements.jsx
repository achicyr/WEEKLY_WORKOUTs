...
export function FeaturesList02Cards({
    ...
    return <ul>
        {$datas?.map((item,i) => <li style={{"backgroundImage":"url("+(item?.bg||"next.svg")+")"}}>
            <h2>{item.title}</h2>
            <ul>
                {item?.list?.map((listItem,i) => <li>
                    {listItem.imgSrc 
...
export function PricingList01({
    ...
    return <ul>
        {$datas.map((item,i) => <li>
            ...
            <ul>{item.list.map((item_,i) => <li>{item_}</li>)}</ul>
...
export function PricingsList02TableCompare({
    ...
    return <table>
        <thead>
            <tr>
                <th></th>
                {options.map((item,i) => <th>{item}</th>)}
            </tr>
        </thead>
        <tbody>
            {$datas.map((item,i) => item==""
                ? <tr className='sep'></tr>
                : <tr><th>{Object.keys(item)[0]}</th>{item[Object.keys(item)[0]].map((item_,i_) => <td>{item_&&<svg><SvgPaths viewBox="0 0 16 16" pathsArray={["M13.854 3.646a.5.5 0 0 1 0 .708l-7 7a.5.5 0 0 1-.708 0l-3.5-3.5a.5.5 0 1 1 .708-.708L6.5 10.293l6.646-6.647a.5.5 0 0 1 .708 0z"]} /></svg>}</td>)}</tr>
            )}
        </tbody>
    </table>
}
